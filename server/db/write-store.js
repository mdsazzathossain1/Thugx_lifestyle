...
"        const n = Number(iv);",
"        if ('$gte' in val && n < Number(val['$gte'])) return false;",
"        if ('$lte' in val && n > Number(val['$lte'])) return false;",
"        if ('$gt' in val && n <= Number(val['$gt'])) return false;",
"        if ('$lt' in val && n >= Number(val['$lt'])) return false;",
"        continue;",
"      }",
"    }",
...
fs.writeFileSync(path.join(__dirname, '..', 'db', 'store.js'), storeContent, 'utf8');
console.log('store.js written with', storeContent.split('\n').length, 'lines');
